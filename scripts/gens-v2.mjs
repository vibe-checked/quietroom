...
export function rainGenerator() {
  ...
  return () => {
    ...
    dropEnv *= 0.7;
    // Was 1.6/0.9 - regularly peaked past +-2, forcing heavy soft-clip
    // limiting on nearly one sample in six. Scaled down to stay close
    // to the +-1 range under normal conditions.
    return hp * 1.0 + drop * 0.55;
  };
}
...
export function campfireGenerator() {
  ...
  return () => {
    ...
    popEnv *= 0.8;
    // Was base+pop with popEnv up to 1.2 - shaved the pop ceiling so a
    // hot crackle doesn't stack with a base peak into hard-clip territory.
    return base + pop * 0.85;
  };
}
